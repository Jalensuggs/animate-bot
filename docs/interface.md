...
The customiser has its own shareable fragment:
`#forme=capsule&couleur=bleu&expression=heureux`. These three values are validated
against the catalogues before they reach the engine. Opening such a link selects
the customiser and restores every valid value; an unknown value is ignored rather
than replacing the current choice. The generated link replaces any player
fragment instead of mixing the two URL contracts.
