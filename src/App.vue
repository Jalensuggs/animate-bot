...
window.addEventListener('hashchange', () => {
  ...
  if (next.partage) {
    if (next.partage.shape) shape.value = next.partage.shape
    if (next.partage.color) color.value = next.partage.color
    if (next.partage.expression) expression.value = next.partage.expression
...
const shape = ref<ShapeId>(
  initial.partage?.shape ??
    (stored('forme', DEFAULT_SHAPE, (v) => SHAPE_BY_ID.has(v)) as ShapeId)
)
const color = ref<ColorId>(
  initial.partage?.color ??
    (stored('couleur', DEFAULT_COLOR, (v) => COLOR_BY_ID.has(v)) as ColorId)
)
const expression = ref<ExpressionId>(
  initial.partage?.expression ??
    (stored('expression', DEFAULT_EXPRESSION, (v) => EXPRESSION_BY_ID.has(v)) as ExpressionId)
)
 ...
 * c'est une marque. La constante est ecrite ici plutot que tiree de
 * `t('app.name')`, qui n'a pas la meme casse.
 */
const NOM = 'ANIMATE BOT'
...
/** Copie un lien qui restaure exactement l'avatar courant. */
async function partageAvatar() {
  if (etatExport.value === 'occupe') return
  clearTimeout(confirmation)
  etatExport.value = 'occupe'
  try {
    const lien = lienPartage(location.href, {
      shape: shape.value,
      color: color.value,
      expression: expression.value
    })
    await copieTexte(lien)
  ...
  <template v-else>
    ...
    >
      ...
      >
        ...
        >
          <ExportBar :etat="etatExport" @exporter="exporte" @partager="partageAvatar" />
        </div>
